...
export function generateQuestion(skillKey: string, difficulty: number, seed: string): GeneratedQuestion {
  ...
  const make = (kind: string, answer: number, explanationKey: string, left?: number, right?: number, visual?: QuestionPresentation["visual"]): GeneratedQuestion => ({
    presentation: { kind, interaction, promptKey: `questions.${kind}`, left, right, visual, choices: interactionChoices(interaction, answer, Math.max(max, answer + 5), random), ...timed },
    correctAnswer: String(answer), explanationKey,
  });
  ...
  if (skill.generatorKey === "fraction") { const denominator = [2, 3, 4, 5][randomInt(0, 3, random)]; const numerator = randomInt(1, denominator - 1, random); if (interaction === "matching") { const source = "1/2"; const answer = "2/4"; return { presentation: { kind: "fraction", interaction, promptKey: "questions.matching", matchSource: source, choices: shuffle([answer, "1/3", "3/4", "2/3"], random), visual: { numerator: 1, denominator: 2 }, ...timed }, correctAnswer: answer, explanationKey: "feedback.fractionExplanation" }; } const answer = `${numerator}/${denominator}`; const choices = shuffle([answer, `1/${denominator}`, `${Math.min(denominator - 1, numerator + 1)}/${denominator}`, `${numerator}/${denominator + 1}`], random); return { presentation: { kind: "fraction", interaction, promptKey: "questions.fraction", choices: interaction === "numeric" ? undefined : choices, visual: { numerator, denominator }, ...timed }, correctAnswer: answer, explanationKey: "feedback.fractionExplanation" }; }
