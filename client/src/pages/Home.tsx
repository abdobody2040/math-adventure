...
import { enqueueAnswer, readQueuedAnswers, removeQueuedAnswers, summarizeOfflineSync } from "@/lib/offlineQueue";
...
function RewardShelf({ child }: { child: any }) {
  ...
  const equipItem = trpc.rewards.equipItem.useMutation({ onSuccess: () => refetch() });
  const equipPet = trpc.rewards.equipPet.useMutation({ onSuccess: () => refetch() });
  ...
  const equippedItems = new Set((data?.inventory ?? []).filter((item: any) => item.equipped).map((item: any) => item.itemKey));
  const equippedPets = new Set((data?.pets ?? []).filter((pet: any) => pet.equipped).map((pet: any) => pet.petKey));
  return <section className="reward-shelf"><div className="section-title"><div><p>{t("rewards.collection")}</p><h2>{t("rewards.companions")}</h2></div><Backpack size={20} /></div><div className="reward-grid">{(data?.petCatalog ?? []).map((pet: any) => <article key={pet.key} className={equippedPets.has(pet.key) ? "is-equipped" : ""}><PetIllustration petKey={pet.key} /><b>{t(pet.titleKey)}</b><small>{t(pet.descriptionKey)}</small>{!ownedPets.has(pet.key) ? <button disabled={unlock.isPending} onClick={() => unlock.mutate({ childId: child.id, petKey: pet.key })}>{`${number(pet.unlockCoins)} ${t("dashboard.coins")}`}</button> : <button disabled={equippedPets.has(pet.key) || equipPet.isPending} onClick={() => equipPet.mutate({ childId: child.id, petKey: pet.key })}>{equippedPets.has(pet.key) ? t("rewards.equipped") : t("rewards.equip")}</button>}</article>)}</div><div className="cosmetic-row">{(data?.catalog ?? []).map((item: any) => <button key={item.key} className={equippedItems.has(item.key) ? "is-equipped" : ""} disabled={(!ownedItems.has(item.key) && redeem.isPending) || (ownedItems.has(item.key) && equippedItems.has(item.key))} onClick={() => ownedItems.has(item.key) ? equipItem.mutate({ childId: child.id, itemKey: item.key }) : redeem.mutate({ childId: child.id, itemKey: item.key })}><RewardIllustration itemKey={item.key} /><b>{t(item.titleKey)}</b><small>{!ownedItems.has(item.key) ? `${number(item.costCoins)} ${t("dashboard.coins")}` : equippedItems.has(item.key) ? t("rewards.equipped") : t("rewards.equip")}</small></button>)}</div></section>;
}

function EquippedExplorer({ child, dashboard, scene = false }: { child: any; dashboard: any; scene?: boolean }) {
  const cosmetics = dashboard?.equippedCosmeticKeys ?? [];
  return <div className={`equipped-explorer ${scene ? "is-scene" : ""}`}><ExplorerPortrait avatarKey={child.avatarKey} size="lg" alt={child.displayName} eager />{cosmetics.map((itemKey: string) => <span className={`equipped-cosmetic ${itemKey}`} key={itemKey}><RewardIllustration itemKey={itemKey} /></span>)}{dashboard?.equippedPetKey && <span className="equipped-pet"><PetIllustration petKey={dashboard.equippedPetKey} /></span>}</div>;
}
...
function ChildDashboard({ child, dashboard, curriculum, setScreen, startLesson, startBoss }: { child: any; dashboard: any; curriculum: any; setScreen: (screen: Screen) => void; startLesson: (skillKey: string, battle?: boolean) => void; startBoss: (worldKey: string) => void }) {
  ...
  return <main id="top" className="app-main child-home">
    <section className="welcome-row"><div><p className="eyebrow"><Sparkles size={15} />{t("common.welcome")}</p><h1>{t("dashboard.greeting")}</h1></div><EquippedExplorer child={child} dashboard={dashboard} /></section>
    ...
    <section className="continue-panel"><div className="continue-copy"><p className="eyebrow"><Play size={15} />{t("lesson.sessionIntro")}</p><h2>{t("dashboard.continueTitle")}</h2><p>{t("dashboard.continueDescription")}</p><button className="primary-button" onClick={() => startLesson(currentSkill)}><span>{t("common.continue")}</span><ArrowRight size={18} /></button></div><div className="continue-scene"><span className="scene-cloud cloud-one" /><span className="scene-cloud cloud-two" /><span className="scene-sun" /><div className="scene-hill" /><div className="scene-avatar"><EquippedExplorer child={child} dashboard={dashboard} scene /></div><span className="scene-star one">✦</span><span className="scene-star two">✦</span></div></section>
    <section className="dashboard-grid"><article className="quest-card"><div className="card-heading"><span className="card-icon mint"><Compass size={19} /></span><div><p>{t("dashboard.dailyQuest")}</p><h3>{t(dashboard?.dailyQuest?.titleKey ?? "quests.dailyFive")}</h3></div></div><p className="muted">{t("dashboard.questDescription", { target: number(dashboard?.dailyQuest?.target ?? 5) })}</p><div className="progress-label"><span>{t("dashboard.progress", { progress: number(dashboard?.dailyQuest?.progress ?? 0), target: number(dashboard?.dailyQuest?.target ?? 5) })}</span><b>{Math.round(((dashboard?.dailyQuest?.progress ?? 0) / (dashboard?.dailyQuest?.target ?? 5)) * 100)}%</b></div><div className="meter"><i style={{ width: `${((dashboard?.dailyQuest?.progress ?? 0) / (dashboard?.dailyQuest?.target ?? 5)) * 100}%` }} /></div><div className="quest-rewards"><span><Zap size={14} />{number(dashboard?.dailyQuest?.rewardXp ?? 25)} {t("dashboard.xp")}</span><span><Coins size={14} />{number(dashboard?.dailyQuest?.rewardCoins ?? 10)}</span></div></article><article className="quest-card weekly-quest"><div className="card-heading"><span className="card-icon sun"><Award size={19} /></span><div><p>{t("dashboard.weeklyQuest")}</p><h3>{t(dashboard?.weeklyQuest?.titleKey ?? "quests.weeklyPractice")}</h3></div></div><p className="muted">{t("dashboard.questDescription", { target: number(dashboard?.weeklyQuest?.target ?? 5) })}</p><div className="progress-label"><span>{t("dashboard.progress", { progress: number(dashboard?.weeklyQuest?.progress ?? 0), target: number(dashboard?.weeklyQuest?.target ?? 5) })}</span><b>{Math.round(((dashboard?.weeklyQuest?.progress ?? 0) / (dashboard?.weeklyQuest?.target ?? 5)) * 100)}%</b></div><div className="meter"><i style={{ width: `${((dashboard?.weeklyQuest?.progress ?? 0) / (dashboard?.weeklyQuest?.target ?? 5)) * 100}%` }} /></div><div className="quest-rewards"><span><Zap size={14} />{number(dashboard?.weeklyQuest?.rewardXp ?? 80)} {t("dashboard.xp")}</span><span><Coins size={14} />{number(dashboard?.weeklyQuest?.rewardCoins ?? 35)}</span></div></article>
...
function LessonExperience({ childId, skillKey, isBattle, exit }: { childId: string; skillKey: string; isBattle: boolean; exit: () => void }) {
  ...
  const [syncMessage, setSyncMessage] = useState<"saved" | "rejected" | null>(null);
  ...
  useEffect(() => {
    const flush = () => {
      ...
      syncAnswers.mutate({ childId, operations }, { onSuccess: data => {
        const summary = summarizeOfflineSync(data.results);
        removeQueuedAnswers(childId, data.results.filter(item => item.status !== "rejected").map(item => item.idempotencyKey));
        setSyncMessage(summary.rejected ? "rejected" : summary.processed || summary.duplicates ? "saved" : null);
      } });
    };
